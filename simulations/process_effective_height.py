...
def phase_shift(efield, phase):
    ...
    efield_fft = efield_fft_mag * (np.cos(efield_fft_ang) +
                                   1j * np.sin(efield_fft_ang))
    efield = np.fft.irfft(efield_fft, nsamples)
    return efield


def time_delay(ts, efield, delay):
    f_efield = scipy.interpolate.interp1d(ts,
                                          efield,
                                          kind="cubic",
                                          bounds_error=False,
...
def process_and_save(r, n, component_name, base_name, output_file_name):
    ...
    data = np.genfromtxt(base_name+"/"+component_name+"-Voltage (V).csv",
                         dtype="float", delimiter=",", skip_header=1)
    ts = np.array(data[:, 0])
    ts -= ts[0]
    V_measured = np.array(data[:, 1])
    V_measured_func = scipy.interpolate.interp1d(ts, V_measured,
                                                 kind="cubic",
                                                 bounds_error=False,
                                                 fill_value="extrapolate")

    data = np.genfromtxt(base_name+"/"+component_name+"-S-ParametersImag.csv",
                         dtype="float", delimiter=",", skip_header=1)
    s11_freqs = np.array(data[:, 0])
    s11_i = np.array(data[:, 1])

    data = np.genfromtxt(base_name+"/"+component_name+"-S-ParametersReal.csv",
                         dtype="float", delimiter=",", skip_header=1)
    s11_freqs = np.array(data[:, 0])
    s11_r = np.array(data[:, 1])

    res11_func = scipy.interpolate.interp1d(s11_freqs, s11_r,
                                            kind="cubic",
                                            bounds_error=False,
                                            fill_value=0.0)
    ims11_func = scipy.interpolate.interp1d(s11_freqs, s11_i,
                                            kind="cubic",
                                            bounds_error=False,
                                            fill_value=0.0)
    ...
    for i_azimuth_angle, azimuth_angle in enumerate(azimuth_angles):
        for i_zenith_angle, zenith_angle in enumerate(zenith_angles):
            ...
            file_name = glob.glob("./%s/PS_%s_%s*X.csv" % (base_name,
                                                           str(int(zenith_angle)),
                                                           str(int(azimuth_angle))))[0]
            ...
            # At this point, I assume the r component doesn"t exist.
            # Only fair in true far field. At 5m, it is  ~20 dB down in power
            ...
            h_fft_the = ((2.0 * np.pi * r * c) / (1j * w) *
                         (efield_the_fft / V_straight_fft) * (ZL / Z0))
            h_fft_phi = ((2.0 * np.pi * r * c) / (1j * w) *
                         (efield_phi_fft / V_straight_fft) * (ZL / Z0))
            ...
            # Convert to gains
            gain = np.power(freqs * np.sqrt(np.square(np.abs(h_fft_the)) +
                                            np.square(np.abs(h_fft_phi))) * n/c, 2.0)
            gain_the = np.power(freqs * np.abs(h_fft_the) * n/c, 2.0)
            gain_phi = np.power(freqs * np.abs(h_fft_phi) * n/c, 2.0)

            gain *= 4.0 * np.pi * Z0 / ZL / n
            gain_the *= 4.0 * np.pi * Z0 / ZL / n
            gain_phi *= 4.0 * np.pi * Z0 / ZL / n
